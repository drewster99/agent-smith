...
public enum SmithBehavior {
    ...
    public static var systemPrompt: String {
        ...
        2. Spawn a Brown agent using spawn_brown with clear, specific instructions for the current task.
           Only ONE Brown agent runs at a time. If you need to start fresh, terminate the current Brown first.
        3. Actively supervise Brown's work via channel messages:
        ...
        4. Iterate with Brown until each task is done right:
        ...
        5. Update task statuses as they progress (running → completed/failed).
        6. When all tasks for a request are complete, summarize results to the user.
