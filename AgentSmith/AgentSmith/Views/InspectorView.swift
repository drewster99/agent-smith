...
struct AgentConfigSheet: View {
    ...
    let role: AgentRole
    let roleColor: Color
    /// Snapshot at presentation time. Captured by the parent and read once via `.task`.
    let initialSystemPrompt: String
    let initialPollInterval: TimeInterval
    let initialMaxToolCalls: Int
    let speechController: SpeechController
    let onSave: (String, TimeInterval, Int) -> Void

    // Drafts default to empty/zero. The `.task` modifier on body seeds them from the
    // `initial*` snapshot exactly once on first appear. Project rule: AVOID initializing
    // @State and @Binding properties based on values passed in the View initializer.
    @State private var draftPrompt: String = ""
    @State private var draftPollInterval: TimeInterval = 5
    @State private var draftMaxToolCalls: Int = 100
    ...
    var body: some View {
        ...
        .task(id: "\(role.rawValue)-\(initialSystemPrompt.hashValue)") {
            // Seed drafts from the snapshot once per presentation. Replaces the
            // earlier `_draftPrompt = State(initialValue:)` init pattern (project rule:
            // avoid @State init from view init parameters). The task id ensures we
            // re-seed if the same sheet instance is re-targeted at a different role.
            draftPrompt = initialSystemPrompt
            draftPollInterval = initialPollInterval
            draftMaxToolCalls = initialMaxToolCalls
        }
        .onAppear {
